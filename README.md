...
const { httpLogger } = escriba({
  ...
  httpConf: {
    ...
    propsToParse: {
      request: {
         'id': String,
         'body.document_number': parseInt,
      },
      response: {
         'body.customer.id': parseInt  
      }
    }
  }
})
...
The `propsToParse` attribute is responsible to parse any atribute based on a path, the parsing works by providing a valid javascript native Function (e.g String, parseInt etc).
